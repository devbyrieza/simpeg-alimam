...
import { 
  ...
  PlusCircle, Menu, X, ScrollText, Users
} from "lucide-react";
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 flex-col md:flex-row">
      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between bg-primary-950 p-4 text-white shadow-md z-40 sticky top-0">
        <div className="flex items-center gap-2 font-black text-sm">
          <ScrollText className="w-5 h-5 text-gold-400" />
          <span>Office Al-Imam</span>
        </div>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-primary-950 to-primary-900 text-white flex flex-col shadow-2xl border-r border-primary-800 transition-transform duration-300 ease-in-out

...
function parseSqlDump(filePath) {
    ...
    // Reverse so we match the LATEST duplicate first (which has the complete data)
    return pendaftars.reverse();
}
...
function processData() {
    ...
    const students = [];
    let globalNo = 1;
    for (let sheetName of wbSource.SheetNames) {
        const sheetSource = wbSource.Sheets[sheetName];
        ...
        for (let i = 0; i < rawSource.length; i++) {
            ...
            if (row && row[1]) {
                ...
                if (nama.toLowerCase() === 'nama santri' || nama.toLowerCase().includes('data nomor induk')) continue; 
                students.push({
                    no: globalNo++,
                    nama: nama,
                    jenjang: row[2] || sheetName,
                    nis: row[3]
                });
            }
        }
    ...
    for (let i = 0; i < students.length; i++) {
        const s = students[i];
        const sNamaLower = s.nama.toLowerCase().replace(/[^a-z0-9 ]/g, ''); 
        ...
        if (pendaftar) {
            ...
            // Just force L since it's a boys school and user said "harusnya L semua"
            jk = 'L';
    ...
    const fileTarget = "c:/Users/itpua/Dev/Work/al-andalus/safina-keuangan/Data Santri Al Imam (Final).xlsx";

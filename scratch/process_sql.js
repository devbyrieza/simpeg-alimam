...
function processData() {
    ...
    for (let i = 0; i < students.length; i++) {
        ...
        let pendaftar = pendaftars.find(p => p.nama_lengkap.toLowerCase() === sNamaLower);
        
        // try partial match
        if (!pendaftar) {
            pendaftar = pendaftars.find(p => p.nama_lengkap.toLowerCase().includes(sNamaLower) || sNamaLower.includes(p.nama_lengkap.toLowerCase()));
        }
